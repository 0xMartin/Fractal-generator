...
public class Julius implements Fractal
{
    public float status;
    ...
    public Julius(final int iterations, final Range Real, final Range Imaginar, final Dimension size) {
    ...
    private void render(final Graphics2D g2, final int it, final Range re, final Range im, final Dimension size, final double[] data) {
        ...
        while (this.t1.status != 100.0f || this.t2.status != 100.0f || this.t3.status != 100.0f || this.t4.status != 100.0f) {
            ...
            try {
                ...
                Logger.getLogger(Julius.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    private class SubThread implements Runnable
    {
        ...
        public void run() {
            ...
            this.renderFractal(g2, this.it, this.re, this.im, this.size, this.Re, this.Im);
            this.status = 100.0f;
        }
        
        public void renderFractal(final Graphics2D graphics, final int iterations, final Range Real, final Range Imaginar, final Dimension size, final double re, final double im) {
            long[][] iter = null;
            try {
            ...
            for (int x = 0; x < size.width; ++x) {
                ...
                for (int y = 0; y < size.height; ++y) {
                    ...
                    final double CIm = Imaginar.START + y * ImaginarPerOne;
                    double X;
                    double Y;
                    int iteration;
                    double x_new;
                    for (X = CRe, Y = CIm, iteration = 0; X * X + Y * Y <= 4.0 && iteration < iterations; Y = 2.0 * X * Y + im, X = x_new, ++iteration) {
                        x_new = X * X - Y * Y + re;
                    }
                    try {
                        iter[x][y] = iteration;
